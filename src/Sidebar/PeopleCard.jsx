...
export function PeopleCard({ data, title }) {
  if (!data || data.length === 0) {
    return <p className="text-center text-gray-500">No trending items available.</p>;
  }
  ...
  return (
    <div className="w-full flex flex-wrap gap-6 justify-center p-4">
      {data.map((d, i) => (
        ...
        >
          {/* Image Section */}
          <Link to={`/${data.media_type || title}/details/${d.id}`} className="relative block w-full h-[400px] rounded-t-lg overflow-hidden">
            <img
              className="w-full h-full object-cover"
              src={`https://image.tmdb.org/t/p/original${d.backdrop_path || d.poster_path || d.profile_path}`}
              alt={d.name || d.title || d.original_name || d.original_title || "Profile"}
            />
          ...
          <div className="p-4 text-white">
            <h1 className="text-lg font-semibold truncate">
              {d.name || d.title || d.original_name || d.original_title || "Name not available"}
            </h1>
          </div>
        </div>
      ))}
